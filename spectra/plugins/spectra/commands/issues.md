...
If the script exits with error (missing tasks.md), display an error message instructing the user to run `/spectra:tasks` first, then stop execution.
...
After authentication, run `/spectra:issues` again.
```
...
| File Path | Text inside final `()` | "spectra/commands/implement.md" |
...
| Feature | `feature:{name}` | `feature:spectra-plugin` | `D93F0B` (red) |
...
*Generated by spectra `/issues` command*
...
1. **Wait and Resume** - Rate limits typically reset within an hour
   - Run `/spectra:issues --resume` after the limit resets
...
Run `/spectra:issues --resume` to continue after resolving the network issue.
```
...
*Run `/spectra:issues --resume` to resume if interrupted*
```

## Usage

```
/spectra:issues [options]
```

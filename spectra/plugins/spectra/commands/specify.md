...
Defer lower-priority ambiguities to the `/spectra:clarify` command for later resolution.
...
- Inform user they can run `/spectra:clarify` to address these questions
...
- Mention that additional clarifications can be addressed with `/spectra:clarify`
...
- Report the count and recommend:
  ...
  - Run `/spectra:clarify` to resolve existing questions first
...
3. Note that spec is ready for `/spectra:plan` command
...
- Recommend running `/spectra:clarify` to improve quality
...
2. **Run `/spectra:clarify`**: This command will help resolve open questions interactively

3. **Then proceed to `/spectra:plan`**: After clarifications are resolved, generate the implementation plan
```
...
2. **Run `/spectra:plan`**: Generate the implementation plan with architecture and design decisions
```

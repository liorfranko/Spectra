...
If the check fails, use the `/spectra:specify` skill first to create the specification.
...
| Test Location | [e.g., tests/, __tests__/, spectra/tests/] |
...
1. **Project-specific**: `.spectra/memory/constitution.md`
2. **Plugin default**: `$CLAUDE_PLUGIN_ROOT/memory/constitution.md`

```bash
# Check for constitution file
CONSTITUTION_PATH=""
if [ -f ".spectra/memory/constitution.md" ]; then
    CONSTITUTION_PATH=".spectra/memory/constitution.md"
...
|------|---------|------------|
| spec.md | Requirements and acceptance criteria | /spectra:specify |
| research.md | Technology decisions and findings | /spectra:plan |
| data-model.md | Data structures and schemas | /spectra:plan |
| plan.md | Technical context and implementation guidance | /spectra:plan |
| quickstart.md | Setup and usage guide | /spectra:plan |
| tasks.md | Implementation task breakdown | /spectra:tasks |
```
...
**Alternative Actions**:
- `/spectra:analyze` - Perform cross-artifact consistency analysis
...
Next step: Run /spectra:tasks to generate implementation tasks
```

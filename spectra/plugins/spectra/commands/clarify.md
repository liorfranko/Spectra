...
If the check fails, use the `/spectra:specify` command first to create the specification.
...
If `SPEC_EXISTS` is false, display an error message instructing the user to run `/spectra:specify` first, then stop execution.
...
|---------|------|--------|---------|
| {previous entries...} |
| {increment version} | {current date} | Claude (spectra/clarify) | Resolved {N} clarification questions: {brief summary} |
```
...
Run `/spectra:clarify` again to address remaining questions, or proceed
with `/spectra:plan` using the suggested defaults for unresolved items.
```
...
If all criteria met:
  ...
  recommendation = "Run /spectra:plan to generate implementation plan"

If markers remain but < 3:
  completeness = "MOSTLY_COMPLETE"
  recommendation = "Proceed to /spectra:plan or run /spectra:clarify again"

If markers >= 3 or blocking issues exist:
  completeness = "INCOMPLETE"
  recommendation = "Run /spectra:clarify again to resolve remaining issues"
```
...
Options:
  /spectra:plan      - Proceed with implementation planning
  /spectra:clarify   - Resolve remaining questions

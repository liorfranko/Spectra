...
If the prerequisite check fails, run `/spectra:specify` first to create the specification.
...
```
Note: Only spec.md found. Run /spectra:plan and /spectra:tasks for full cross-artifact analysis.
```
...
- `/spectra:implement` - Begin implementation
...
- Run `/spectra:clarify` to resolve open questions
- Then `/spectra:analyze` again to verify fixes

**If FAIL:**
Critical issues must be resolved before proceeding:
- Address all high severity issues listed above
- Run `/spectra:analyze` again after fixes
- Do not proceed to implementation until PASS status achieved

---

*Report generated by spectra analyze command*
*Run `/spectra:analyze` again after making changes to verify fixes*
```
...
**Note**: This command is read-only and does not modify any files. To fix identified issues, manually update the appropriate artifacts and run `/spectra:analyze` again to verify.

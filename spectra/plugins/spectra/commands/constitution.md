...
|---------|------|---------|--------|
| 1.0.0 | {TODAY} | Initial constitution established | {User or "Claude (spectra)"} |
...
```bash
# Create .spectra/memory directory if it doesn't exist
mkdir -p ".spectra/memory"
...
```markdown
| 1.1.0 | {TODAY} | Added principle: {principle_title} | Claude (spectra) |
```
...
- `/spectra:plan` - Technical planning will verify compliance
- `/spectra:implement` - Implementation will follow constraints
- `/spectra:review-pr` - Reviews will verify adherence
...
```
/spectra:constitution add "Your new principle text here"
```
...
| File | Description |
|------|-------------|
| `.spectra/memory/constitution.md` | Project constitution document |
...
```
/spectra:constitution [action] [arguments]
```

#!/usr/bin/env bash
# spectra/scripts/setup-hooks.sh - Initialize hooks in user's project
# Copies hook templates to .spectra/ directory and creates necessary configuration
set -euo pipefail
...
Initialize spectra hooks in the current project.
Creates .spectra/ directory with hooks and memory infrastructure.
...
WHAT GETS CREATED:
    .spectra/hooks/           Hook scripts directory
    .spectra/memory/          Persistent memory storage
    .spectra/templates/       Template files for specs/plans
...
---
*Last updated: automatically by spectra*
EOF
...
main() {
    ...
    log_info "Setting up spectra hooks..."

#!/usr/bin/env bash
# spectra/scripts/check-prerequisites.sh - Validate prerequisites for Spectra commands
...
Validate prerequisites for Spectra commands and report available documents.
...
check_git() {
    if ! command -v git &>/dev/null; then
        if [[ "$JSON_OUTPUT" == "true" ]]; then
            ...
            error "git is not installed. Please install git to use Spectra."
        fi
        exit 1
    fi
}
...
check_git_repo() {
    if ! git rev-parse --is-inside-work-tree &>/dev/null; then
        if [[ "$JSON_OUTPUT" == "true" ]]; then
            ...
            error "Not in a git repository. Spectra requires a git repository."
        fi
        exit 1
    fi
}

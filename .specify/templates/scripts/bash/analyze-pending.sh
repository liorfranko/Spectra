#!/usr/bin/env bash
# Continuous Learning - Background Analyzer
#
# Processes pending sessions and creates atomic instincts from detected corrections.
# Run this manually or via cron to analyze sessions queued by session-end.sh
#
# Usage:
#   ./analyze-pending.sh                  # Process all pending sessions
#   ./analyze-pending.sh --list           # List pending sessions
#   ./analyze-pending.sh --decay          # Apply confidence decay to stale instincts
#   ./analyze-pending.sh --promote        # Promote high-confidence instincts to skills
...
reinforce_instinct() {
    ...
    local new_conf
    new_conf=$(echo "$current_conf + 0.1" | bc -l)
    # Ensure leading zero for proper JSON (bc outputs ".6" not "0.6")
    new_conf=$(printf "%.2f" "$new_conf")
    if (( $(echo "$new_conf > $CONFIDENCE_MAX" | bc -l) )); then
    ...
    log_info "Reinforced instinct: $(basename "$instinct_file") → confidence $new_conf (×$new_count)"
}
...
apply_decay() {
    ...
    while IFS= read -r instinct_file; do
        ...
        if [[ "$weeks_stale" -gt 0 ]]; then
            ...
            local decay_amount
            decay_amount=$(echo "$weeks_stale * $DECAY_PER_WEEK" | bc -l)
            local new_conf
            new_conf=$(echo "$current_conf - $decay_amount" | bc -l)
            # Ensure leading zero for proper JSON
            new_conf=$(printf "%.2f" "$new_conf")

            # Archive if confidence drops below threshold
            if (( $(echo "$new_conf < 0.1" | bc -l) )); then
            ...
            elif (( $(echo "$new_conf < $current_conf" | bc -l) )); then
                # Apply decay
                local temp_file="${instinct_file}.tmp"
                sed "s/\"confidence\"[[:space:]]*:[[:space:]]*[0-9.]*/\"confidence\": $new_conf/" "$instinct_file" > "$temp_file"
                mv "$temp_file" "$instinct_file"

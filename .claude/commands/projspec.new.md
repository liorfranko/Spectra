...
```
/projspec.new <spec-name>
```

**Arguments:**
- `spec_name` (required) - Name for the spec in kebab-case (e.g., `user-auth`, `payment-v2`)
...
- `/projspec.init` - Initialize ProjSpec before creating specs
- `/projspec.spec` - Create specification from the brief (next step)
- `/projspec.status` - View all active specs

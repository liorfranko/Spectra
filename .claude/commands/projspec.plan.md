...
Reads `spec.md` and creates an implementation `plan.md` with technical context, component architecture, and build order.
...
- `/projspec.spec` - Create specification document (previous step)
- `/projspec.tasks` - Generate implementation tasks (next step)
- `/projspec.status` - View all active specs and their phases

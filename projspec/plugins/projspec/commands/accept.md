...
Validate that a feature is ready for merge by checking task completion, running quality gates, and confirming all acceptance criteria are met. This is the final gate before `/projspec:merge`.
...
Complete the remaining tasks and run `/projspec:accept` again.
```
**STOP execution.**
...
**Next Step:** Run `/projspec:merge` to merge this feature into {BASE_BRANCH}.
...
After resolving issues, run `/projspec:accept` again.
{End if}
```
...
| Condition | Behavior |
|-----------|----------|
| All checks pass | Report success, recommend `/projspec:merge` |
...
# Specify actor name
/projspec:accept --actor "John Doe"
```

## Notes

- This command is a gate before `/projspec:merge`

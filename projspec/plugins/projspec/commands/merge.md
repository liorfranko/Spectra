...
Merge a completed feature branch into the base branch (main/master) and optionally clean up the associated worktree and feature branch. This is the final step in the projspec workflow after `/projspec:accept`.
...
- Feature should have passed `/projspec:accept` (recommended)
...
After resolving, run `/projspec:merge` again.
```
**STOP execution.**
...
> ```bash
> /projspec:merge {original_args_without_dry_run}
> ```
{Else:}
---
...
/projspec:merge --target develop --push
...
This command is designed to be the final step in the projspec workflow:

```
/projspec:specify  →  Define the feature
/projspec:plan     →  Create implementation plan
/projspec:tasks    →  Break into tasks
/projspec:implement →  Execute tasks
/projspec:review-pr →  Code review
/projspec:accept   →  Validate readiness  ← Run this first
/projspec:merge    →  Merge and cleanup   ← You are here
```

## Notes

- Always run `/projspec:accept` before `/projspec:merge`

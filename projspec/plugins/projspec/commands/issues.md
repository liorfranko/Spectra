...
After authentication, run `/speckit.issues` again.
```
...
| Feature | `feature:{name}` | `feature:speckit-plugin` | `D93F0B` (red) |
...
*Generated by SpecKit `/issues` command*
...
1. **Wait and Resume** - Rate limits typically reset within an hour
   - Run `/speckit.issues --resume` after the limit resets
...
Run `/speckit.issues --resume` to continue after resolving the network issue.
```
...
*Run `/speckit.issues --resume` to resume if interrupted*
```

## Usage

```
/speckit.issues [options]
```

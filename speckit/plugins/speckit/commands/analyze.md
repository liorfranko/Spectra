...
If the prerequisite check fails, run `/speckit.specify` first to create the specification.
...
```
Note: Only spec.md found. Run /speckit.plan and /speckit.tasks for full cross-artifact analysis.
```
...
- Then `/speckit.analyze` again to verify fixes
...
*Run `/speckit.analyze` again after making changes to verify fixes*
```
...
**Note**: This command is read-only and does not modify any files. To fix identified issues, manually update the appropriate artifacts and run `/speckit.analyze` again to verify.
